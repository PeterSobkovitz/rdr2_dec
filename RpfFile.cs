using AnimKit.Core.CodeWalker.Core.GameFiles;
using System;
...
namespace CodeWalker.GameFiles
{
    ...
    public class RpfFile
    {
        ...
        internal static extern int OodleLZ_Decompress(
            byte[] Buffer,
            long BufferSize,
            byte[] OutputBuffer,
        ...
        private static uint Decompress(
            byte[] buffer,
            uint bufferSize,
            ref byte[] outputBuffer,
            uint outputBufferSize)
        {
            return buffer.Length != 0 && bufferSize > 0U && outputBuffer.Length != 0 && outputBufferSize > 0U ? (uint)OodleLZ_Decompress(buffer, (long)bufferSize, outputBuffer, (long)outputBufferSize, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0U, 0) : 0U;
        }
        public static byte[] Decompress(byte[] data, int decompressedSize)
        {
            byte[] outputBuffer = new byte[decompressedSize];
            if ((long)Decompress(data, (uint)data.Length, ref outputBuffer, (uint)decompressedSize) != (long)decompressedSize)
        ...
        public static RpfResourceFileEntry CreateResourceFileEntry(ref byte[] data, uint ver)
        {
            ...
            if (rsc7 == 0x37435352) 
            {
                System.Diagnostics.Debug.WriteLine("GTAV");
            ...
            else if (rsc7 == 943936338)
            {
                System.Diagnostics.Debug.WriteLine("RDR2");
                
                rpfresourcefilenetry resourceFileentry = rpfresourcefilenetry.Create(ref data, 0U);
                resentry.SystemFlags = resourceFileentry.VirtualFlags;
                resentry.GraphicsFlags = resourceFileentry.PhysicalFlags;
                resentry.SystemSize1 = resourceFileentry.VirtualSize;
                resentry.GraphicsSize1 = resourceFileentry.PhysicalSize;
                System.Diagnostics.Debug.WriteLine(resourceFileentry.CompressorId);
                byte[] outputBuffer = new byte[resourceFileentry.DecompressedSize];
              
        
                switch (resourceFileentry.CompressorId)
                {

                    case 2:

                        data = rpfcompression.Decompress(data, (int)resourceFileentry.DecompressedSize);
        ...
        public static RpfResourceFileEntry CreateResourceFileEntry2(ref byte[] data, uint ver)
        {
            var resentry = new RpfResourceFileEntry();
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class RpfResourceFileEntry : RpfFileEntry
    {
        ...
        public uint SystemSize1 { get; set; }
        public uint GraphicsSize1 { get; set; }
